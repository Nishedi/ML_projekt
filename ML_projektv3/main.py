...
def test(withStandardScaler=True, withVarianceThreshold=True, isPloting=False):
    print("Progowanie? ", withVarianceThreshold, " Skalowanie? ",withStandardScaler)
    data = load_wine()
...
def check_impact_of_variance_and_scaling():
    ...
    num_of_rep = 20
    for i in range(20):
        acc, _,_,_= test(True, True)
        x += acc
    for i in range(20):
        acc, _,_,_= test(True, False)
        y += acc
    for i in range(20):
        acc, _,_,_= test(False, True)
        w += acc
    for i in range(20):
        acc, _,_,_= test(False, False)
        z += acc
